...
public class Canvas(private val strips:Int, private val ledsPerStrip: Int) {
    ...
    class Positions(private val positions:List<Position>, strips: Int, pixels:Int) {
        ...
        init {
            raster = (0..strips).map { strip -> positions
                    .filter { it -> it.strip == strip } }
                    .map { stripPositions -> stripPositions
    ...
    class Position(val strip: Int, val pixel:Int, val x:Int, val y:Int)
}